"""
cno_single_fatband.py

Project one saved CNO orbital onto every Bloch state from a line-mode
...
if cno_index >= cno_orbs.shape[1]:
    raise ValueError(
        f"cno_index={cno_index} out of range; "
        f"cno_orbitals.npy has {cno_orbs.shape[1]} columns."
    )

# ── prepare selected CNO ──────────────────────────────────────────────────────
cno = cno_orbs[:, cno_index].copy()
cno = cno / np.sqrt(np.sum(np.abs(cno) ** 2))

print(f"\nSelected CNO index    : {cno_index}")
print(f"CNO occupation        : {cno_occ[cno_index]:.8e}")
print(f"CNO norm after renorm : {np.sum(np.abs(cno) ** 2):.10f}")
...
# ── compute |<CNO | u_nk>|² for every band and k-point ───────────────────────
# weight[b, k] = |<CNO_alpha | u_nk>|²
# u_nk is the cell-periodic Bloch part reconstructed via IFFT.
# Both CNO and u_nk are unit-normalised, so each weight is in [0, 1].
print(f"\nComputing |<CNO_{cno_index:03d} | u_nk>|² "
      f"for {nbands} bands × {nkpts} k-points...")

weights = np.zeros((nbands, nkpts))
...
for ik in range(1, nkpts + 1):
    ...
    if ik == 1 or ik % 10 == 0 or ik == nkpts:
        print(f"  k-point {ik:4d}/{nkpts}  "
              f"max weight this k: {weights[:, ik - 1].max():.4f}")
...
print(f"  Mean  : {mean_weight:.6e}")
print(f"  Max   : {max_weight:.6f}")

# ── save weights ──────────────────────────────────────────────────────────────
weights_path = fatband_dir / f"cno_{cno_index:03d}_weights.npy"
np.save(weights_path, weights)
print(f"Saved weights to: {weights_path}")

# ── write projection CSV ──────────────────────────────────────────────────────
csv_path = fatband_dir / f"cno_{cno_index:03d}_projection_table.csv"
    ...
    for ib in range(nbands):
        for ik in range(nkpts):
            fh.write(
                f"{ik},{ib},"
                f"{kdistances[ik]:.8f},"
                f"{energies_shifted[ib, ik]:.6f},"
                f"{weights[ib, ik]:.8e}\n"
            )
print(f"Saved projection table to: {csv_path}")

# ── write metadata ────────────────────────────────────────────────────────────
meta_path = fatband_dir / f"cno_{cno_index:03d}_fatband_metadata.txt"
with open(meta_path, "w") as fh:
    fh.write(f"cno_index               : {cno_index}\n")
    fh.write(f"cno_occupation          : {cno_occ[cno_index]:.10e}\n")
    fh.write(f"total_weight            : {total_weight:.10f}\n")
    fh.write(f"mean_weight             : {mean_weight:.6e}\n")
    fh.write(f"max_weight              : {max_weight:.10f}\n")
    ...
    fh.write("NOTE: weight = |<CNO_alpha | u_nk>|^2\n")
    fh.write(
        "NOTE: interpolation (if used) is only for plotting scalar "
        "weights/energies, not wavefunctions.\n"
    )
print(f"Saved metadata to: {meta_path}")

# ── plot: LineCollection colored fatband ──────────────────────────────────────
print("\nPlotting colored fatband (LineCollection)...")
...
for b in range(nbands):
    segs, seg_colors = make_segments(
        kdistances, energies_shifted[b], weights[b],
...
cb.set_label(
    f"CNO {cno_index} overlap "
    r"$|\langle\mathrm{CNO}\,|\,u_{nk}\rangle|^2$",
    fontsize=10,
)
...
ax.set_title(f"CNO {cno_index} projected band structure")
ax.set_xlim(kdistances[0], kdistances[-1])
ax.set_ylim(ylim)

plt.tight_layout()
lc_path = fatband_dir / f"cno_{cno_index:03d}_colored_fatband.png"
fig.savefig(lc_path, dpi=150, bbox_inches="tight")
plt.close(fig)
print(f"Saved colored fatband to: {lc_path}")

# ── plot: scatter debug version ───────────────────────────────────────────────
print("Plotting scatter fatband (debug)...")
...
for b in range(nbands):
    sc = ax2.scatter(
        ...
        c=weights[b], cmap=cmap, s=6,
        vmin=vmin, vmax=vmax, linewidths=0,
    )

if sc is not None:
    cb2 = fig2.colorbar(sc, ax=ax2, pad=0.02)
    cb2.set_label(
        f"CNO {cno_index} overlap "
        r"$|\langle\mathrm{CNO}\,|\,u_{nk}\rangle|^2$",
        fontsize=10,
...
ax2.set_title(f"CNO {cno_index} projected band structure (scatter)")
ax2.set_ylim(ylim)
ax2.set_xlim(kdistances[0], kdistances[-1])

plt.tight_layout()
sc_path = fatband_dir / f"cno_{cno_index:03d}_scatter_fatband.png"
fig2.savefig(sc_path, dpi=150, bbox_inches="tight")
plt.close(fig2)
print(f"Saved scatter fatband to: {sc_path}")
